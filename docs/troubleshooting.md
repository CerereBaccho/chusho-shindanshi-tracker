...
- **エラーメッセージ**: `running scripts is disabled on this system`
- **原因**: Windowsの実行ポリシーがスクリプト実行を禁止している
- **対処法**: 以下のコマンドで実行ポリシーを変更する
...
- **注意**: `Unrestricted` や `Bypass` には設定しない。`RemoteSigned` が安全な最小設定
